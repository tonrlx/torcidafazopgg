-- Script final para configurar Supabase - Remove dependências na ordem correta

-- 1. REMOVER TRIGGERS PRIMEIRO (eles dependem das funções)
...
-- 2. REMOVER FUNÇÕES (agora que os triggers foram removidos)
DROP FUNCTION IF EXISTS update_comment_likes_count() CASCADE;
DROP FUNCTION IF EXISTS is_username_unique(TEXT) CASCADE;
DROP FUNCTION IF EXISTS is_user_banned(UUID) CASCADE;
DROP FUNCTION IF EXISTS is_user_suspended(UUID) CASCADE;
DROP FUNCTION IF EXISTS is_email_banned(TEXT) CASCADE;
DROP FUNCTION IF EXISTS is_user_moderator(UUID) CASCADE;
...
-- 8. CRIAR TRIGGERS (agora que as funções existem)
